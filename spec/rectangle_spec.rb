...
describe Rectangle do
  ...
  context "calculates perimeter in mm" do
    ...
    it "1820 mm for sides 90cm and 10mm " do
      ...
      parameter = Length.new(1820, Unit::MM)
  ...
  context "equality" do
    length = Length.new(30, Unit::CM)
    breadth = Length.new(10, Unit::CM)
    ...
    it 'same perimeter but different sides should not be equal' do
      ...
      rectangle = Rectangle.new(length, breadth)
      rectangle1 = Rectangle.new(new_length, new_length)
      expect(rectangle.perimeter).to eq(rectangle1.perimeter)
      expect(rectangle).to_not eq(rectangle1)
    end
